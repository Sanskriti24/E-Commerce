...
const Footer = () => {
  return (
    <footer className="flex-center pb-4 pt-20">
        <div className="mx-auto flex w-full flex-col gap-14">
            <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
                <Link to="/" className="mb-10 font-bold">Shoppee</Link>
            <div className="flex flex-wrap gap-8 sm:justify-between md:flex-1"> 
            {FOOTER_LINKS.map((col) => (
                <FooterColumn title={col.title} key={col.title}>
                    <ul className="flex flex-col gap-4 regular-14 text-gray-400">
                        {col.links.map((link) => (
                ...
                <FooterColumn title={FOOTER_CONTACT_INFO.title}>
                    {FOOTER_CONTACT_INFO.links.map((link) => (
                        <Link to="/" key={link.label} className="flex gap-4 md:flex-col lg:flex-row">
                            <p className="text-gray-400">{link.label}:</p> <p className="text-gray-400 medium-14">{link.value}</p>
                        </Link>
                    ))}
                </FooterColumn>
            </div>
        ...
        <p className="text-center regular-14 text-gray-400">Shoppee | All rights reserved</p>
    </footer>
  )
};
