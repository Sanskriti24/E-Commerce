    ...
    return (
      <div className='rounded-xl overflow-hidden shadow-lg'>
        <div className='relative flex items-center justify-center group overflow-hidden transition-all duration-100'>
          <Link to={`product/${id}`} className='h-12 w-12 bg-white rounded-full flex items-center justify-center absolute top-1/2 bottom-1/2 !py-2 z-20 scale-0 group-hover:scale-100 transition-all duration-700'>
        ...
        <div className='p-4 overflow-hidden'>
          ...
          <div className='flex gap-5'>
            ...
            <div className='text-orange-400 font-bold line-through'>{old_price}</div>
          </div>
        </div>
      </div>
    );
